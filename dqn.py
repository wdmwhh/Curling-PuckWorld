...
def main():
    ...
    for epoch in range(num_epoch):
        ...
        for step in range(num_step):
            ...
            memory.push((s1, a1, r, s2))
            if step==num_step-1:
                memory.push((s2, a2, 
                    curling.reward(curling.position), ((-1., )*2, )*3))
            
            if len(memory) < batch_size: continue

            with torch.enable_grad():
                s1_batch, a1_batch, r_batch, s2_batch = zip(*memory.sample(batch_size))
                ...
                with torch.no_grad():
                    q_s2_max = target_NN(s2_batch).max(1)[0].detach()
                    q_s2_max *= s2_batch[:, 0]>0

                loss = policy_NN.fit(s1_batch,
                                   a1_batch,
                                   r_batch + q_s2_max * gamma, 
                                   learning_rate = lr,
                                   epochs = 3)
    ...
    torch.save(policy_NN.state_dict(), 'policy.pth')
    curling = Curling()
    ...
    # plt.tight_layout()
    # plt.savefig('curves_20average.png')
    # plt.show()
